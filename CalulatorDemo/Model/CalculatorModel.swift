...
struct CalculatorModel {
    ...
    private struct ArithmeticExpression: Equatable { //I used Equatable because of we need to compare 2 instaces
        var number: Decimal
    ...
    //check decimal value in current number or not
    private var containsDecimalValue: Bool {
        return getNumberString(forNumber: number).contains(".")
    }
    
    // MARK: - Operations
    mutating func setDigit(_ digit: Digits) {
        if containsDecimalValue && digit == .zero {
            carryingZeroCount += 1
...
//    }
    ...
    mutating func setDecimal() {
        if containsDecimalValue { return }
    ...
    private func canAddDigit(_ digit: Digits) -> Bool { //Check wether number will adding or not
